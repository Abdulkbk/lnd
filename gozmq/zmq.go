...
func (c *Conn) Receive(bufs [][]byte) ([][]byte, error) {
	...
	// Otherwise, we'll attempt to reconnect. We use the original address
	// (which may contain a hostname) rather than the resolved IP from
	// RemoteAddr(), so that DNS is re-resolved on reconnection. This
	// ensures we connect to the correct server if its IP has changed
	// (e.g., after a Kubernetes pod restart or Docker network change).
	...
	newConn, err := Subscribe(c.addr, c.topics, c.timeout)
	if err != nil {
